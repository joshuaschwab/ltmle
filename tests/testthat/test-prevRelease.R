...
if (T) {
  test_that("tests from 'create tests to compare versions.R'", {
    ...
    for (j in seq_along(btests)) {
      ...
      passed[j] <- isTRUE(all.equal(current, prev, tolerance=0.001, scale=1, check.attributes=FALSE))
      cat("in test_prevRelease, j = ", j, " passed = ", sum(passed[1:j]), " failed = ", sum(!passed[1:j]), "\n")
      if (!passed[j]) print(all.equal(current, prev, tolerance=0.001, scale=1, check.attributes=FALSE))
      expect_true(passed[j], info=paste(btests[[j]]$info, "j = ", j, "current = ", paste(current, collapse=" "), "prev = ", paste(prev, collapse=" ")))
    }
  })
  ...
  test_that("invalid inputs throw errors SL", {
    ...
    expect_warning(ltmle(data, Anodes = c("A1", "A2"), Ynodes = c("Y1", "Y2"), estimate.time = FALSE, abar = c(1, 1), SL.library = list(g=NULL, Q=c("SL.mean", "SL.NA")), survivalOutcome = T)) #checks coverage on: m$SL.predict <- predict(m, newX.list$newX, X.subset, Y.subset, onlySL = TRUE)$pred
    ...
    ltmle(data, Anodes = c("A1", "A2"), Ynodes = c("Y1", "Y2"), estimate.time = FALSE, abar = c(1, 1), SL.library = list(g=NULL, Q=c("SL.gam")), survivalOutcome = T) #just make sure SL.gam runs (it doesn't like names like (Intercept) so there's code to work around that)
  })
  
  test_that("temp", {
    #move this to 'create tests to compare version after version 0.9-9-3 -- Y before first A/C is new
    
    #final Y before first A/C
    #numeric gform with variance method "iptw"
    n <- 100
    Y0 <- rbinom(n, 1, 0.1)
    Y1 <- rep(NA, n)
    C <- BinaryToCensoring(is.censored = rbinom(n, 1, 0.1))
    Y1[C == "uncensored"] <- rbinom(sum(C == "uncensored"), 1, 0.1)
    Y1[Y0 == 1] <- 1
    C[Y0 == 1] <- NA
    data <- data.frame(Y0, C, Y1)
    #AddTest(fun = "ltmleMSM", args=list(data=data, Anodes=NULL, Cnodes="C", Ynodes=c("Y0", "Y1"), final.Ynodes = c("Y0", "Y1"), regimes=NULL, summary.measures = NULL, working.msm = "Y~1", gform = matrix(0.5, nrow=n, ncol=1), variance.method="iptw", survivalOutcome = T), ver = "0.9-9-3")
    ltmleMSM(data=data, Anodes=NULL, Cnodes="C", Ynodes=c("Y0", "Y1"), final.Ynodes = c("Y0", "Y1"), regimes=NULL, summary.measures = NULL, working.msm = "Y~1", gform = matrix(0.5, nrow=n, ncol=1), variance.method="iptw", survivalOutcome = T)
  })
