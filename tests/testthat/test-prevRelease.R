...
test_that("invalid inputs throw errors SL", {
  ...
  SL.lib <- "SL.gam"
  attr(SL.lib, "return.fit") <- T
  r <- ltmle(data, Anodes = c("A1", "A2"), Ynodes = c("Y1", "Y2"), estimate.time = FALSE, abar = c(1, 1), SL.library = list(g=NULL, Q=SL.lib), survivalOutcome = T) #make sure SL.gam runs (it doesn't like names like (Intercept) so there's code to work around that); also check "return.fit"
  expect_equal(class(r$fit$Q$Y1), "SuperLearner")
})
